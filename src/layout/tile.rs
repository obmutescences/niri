...
enum FocusAnimationState {
    #[default]
    Idle,
    /// The window is performing a focus animation effect.
    Flashing {
        phase: FlashPhase,
    },
}
...
impl<W: LayoutElement> Tile<W> {
    ...
    pub fn advance_animations(&mut self) {
        ...
        if let Some(scale) = &mut self.scale_animation {
            if scale.anim.is_done() {
    ...
    pub fn update_focus_animation(
        ...
        // 1. Check for Focus Animation Trigger
        ...
        if is_focused && !self.was_focused && animation_enabled {
            ...
            let shrink_config = match focus_config.anim.kind {
                ...
                niri_config::animations::Kind::Spring(_) => {
                    // Fast shrink for spring
                    niri_config::Animation {
                        off: false,
                        kind: niri_config::animations::Kind::Easing(
                            niri_config::animations::EasingParams {
        ...
        match self.focus_animation_state {
            FocusAnimationState::Flashing { phase } => {
                if let Some(scale) = &self.scale_animation {
                    if scale.anim.is_done() {
                        match phase {
                            FlashPhase::Shrinking => {
                                // Shrinking done -> Switch to Expanding
                                let expand_config = match focus_config.anim.kind {
                                    niri_config::animations::Kind::Easing(p) => {
                                        let duration = p
                                            .duration_ms
                                            .saturating_sub(p.duration_ms.div_ceil(2));
                                        niri_config::Animation {
                                            off: false,
                                            kind: niri_config::animations::Kind::Easing(
                                                niri_config::animations::EasingParams {
                                ...
                                self.animate_scale(scale.anim.value(), 1.0, expand_config);
                            }
                            FlashPhase::Expanding => {
                                // Expanding done -> Animation complete
    ...
    pub fn render<'a, R: NiriRenderer + 'a>(
        ...
        if let Some(open) = &self.open_animation {
            ...
            let elements =
                self.render_inner(renderer, Point::from((0., 0.)), focus_ring, target, output);
            let elements = elements.collect::<Vec<TileRenderElement<_>>>();
            match scale_anim.offscreen.render(renderer, scale, &elements) {
                Ok((elem, _sync, data)) => {
                    ...
                    self.window().set_offscreen_data(Some(data));
                    scale_anim_elem = Some(elem.into());
                }
                Err(err) => {
        ...
        if open_anim_elem.is_none() && alpha_anim_elem.is_none() && scale_anim_elem.is_none() {
